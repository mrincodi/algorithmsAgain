...
3
The candidate with 1 rating gets 1 candy and candidate with rating cannot get 1 candy 
as 1 is its neighbor. So rating 2 candidate gets 2 candies. In total, 2+1 = 3 candies 
need to be given out.

 * @author mrincodi
 * 2016-08-13
 * Comment: diabolic problem. Grr... This is a perfect example of a problem that looks
 * like recursion at the beginning but that is much easier taking it as a whole. The 
 * hint was fundamental: "Should you start assigning the candies from the least 
 * rating guy to the guy with most rating or vice versa ?"
 *
 */
...
public class DistributeCandy {
	...
	public int candy(ArrayList<Integer> ratings) {
		...
		for ( int i = 0; i < ratings.size(); i++){
			int rank = ratings.get(i);
			if ( !h.containsKey(rank) ) h.put (rank, new ArrayList<Integer>(Arrays.asList(i)));
			else h.get(rank).add(i);
		}

		// Now, get the keys (ranks) and sort them from small to big.
		ArrayList <Integer> rankList = new ArrayList <Integer> ();

		for ( int rank : h.keySet()){
			rankList.add(rank);
		}        
		Collections.sort ( rankList );

		for (int rank : rankList ){
			for ( int pos: h.get(rank) ){
				//Look at the neighbors that have a rank smaller than yours.
				if (pos == 0 ){
					if (ratings.get(1)<ratings.get(0)) candies[0]=candies[1]+1;
					else candies[0]=1;
				}
				else if (pos == candies.length - 1){
					if ( ratings.get ( pos - 1 ) < ratings.get ( pos )) 
						candies [ pos ] = candies[pos - 1]+1;
					else candies[ pos ]=1;
				}
				else {
					if ( ratings.get ( pos -1 ) >= ratings.get ( pos ) &&
							ratings.get ( pos +1 ) >= ratings.get ( pos ) ) {
						candies[pos]=1; 
					}
					if ( ratings.get ( pos -1 ) >= ratings.get ( pos ) &&
							ratings.get ( pos +1 ) < ratings.get ( pos ) ) {
					...
					if ( ratings.get ( pos -1 ) < ratings.get ( pos ) &&
							ratings.get ( pos +1 ) < ratings.get ( pos ) ) {
						candies[pos]=Math.max(candies[pos-1],candies[pos+1]) + 1;
					}
				}
			}
		}
