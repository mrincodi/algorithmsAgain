 ...
 * @author mrincodi
 * 2016-12-28
 * Comment: Second approach, with a single array. Still takes long for InterviewBit.
 *
 */
public class Flip2 {
	public ArrayList<Integer> flip(String A) {
		...
		m[0] = A.charAt(0)=='0'?numOnes+1:numOnes-1;
		int maxValSoFar = m[0];
		int maxLLimitSoFar = 0;
		int maxRLimitSoFar = 0;
		for ( int i = 1; i < A.length(); i++ ){
			char c = A.charAt(i);
			m[i] = c=='0'?m[i-1]+1:m[i-1]-1;
			if (m[i] > maxValSoFar ){
				maxValSoFar=m[i];
				maxRLimitSoFar=i;
			}
